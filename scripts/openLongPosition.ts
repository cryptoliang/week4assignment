...
async function main() {
    ...
    console.log("> starting open long position");
    let usdcAmount = ethers.utils.parseUnits("1000", 6);
    await trader.long(usdcAmount, tEthAddr, 3)

    console.log("-------- finish open long position ----------")
    await logBalance(usdc, user.address)
    await logUnderlying(tEth, trader.address, "WETH", 18);
    await logBorrow(tUsdc, trader.address, "USDC", 6);
}
